  ...
  try {
        ...
        stdout: JSON.stringify(
          {
            ...
            project_path: bundle.manifest.path?.value ?? selected.pathHint,
            started_at: selected.timestamp,
            session_ids: bundle.manifest.includedSessionIds,
    ...
    return {
      ...
      stdout: [
        ...
        `Project path: ${bundle.manifest.path?.value ?? selected.pathHint}`,
        `Started at: ${selected.timestamp}`,
        `Session IDs: ${bundle.manifest.includedSessionIds.join(', ')}`,
